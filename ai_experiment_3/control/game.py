...
class Game:
    ...
    def updateGame(self, playerInput):
        ...
        if (playerInput[1] >= 0.5):
            result = self.sub.checkIfHitByBeam(math.cos(self.player.angle * math.pi/180), math.sin(self.player.angle * math.pi/180))
            if (result):
                # print("Hit!")
    ...
    def trainAI(self, genome, config):
        ...
        actions[0] = (actions[0] % 90) - 45 / self.deltaTime
